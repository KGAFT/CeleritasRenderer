...
namespace RenderingEngine{
    struct AssemblyPipelineUboData {
        int mode;
    };
    ...
    public:
        AssemblyPipeline(VulkanDevice *device, VulkanSwapChain *swapChain, int width, int height) : RenderPipeline(device, swapChain), device(device),
            ...
            endConfig.pushConstantInfos.push_back({VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(AssemblyPipelineUboData)});
    ...
    private:
        ...
        AssemblyPipelineUboData& getData(){
            return data;
        }
