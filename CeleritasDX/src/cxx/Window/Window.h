...
public:
    ...
    static Window *createWindow(int width, int height, const char *title, Monitor *monitor) {
        ...
        if (windowHandle != nullptr) {
            ...
            windowInstance->fullScreen = monitor!=nullptr;
...
private:
    ...
    Window(int width, int height, const char *title, GLFWwindow *window) : title(title), width(width), height(height),
                                                                           windowHandle(window) {}
    ...
    void checkMovementCallbacks() {
        ...
        if (x != 0 || y != 0) {
            for (const auto &element: movementCallbacks) {
                if (element != nullptr) {
                    if (element->getCallbackMode() == currentMode ||
                        element->getCallbackMode() == WINDOW_MOUSE_CALLBACK_BOTH_MODE) {
...
public:
    ...
    void setMonitor(Monitor *monitor) {
        glfwSetWindowMonitor(windowHandle, monitor==nullptr?nullptr:monitor->getMonitor(), 0, 0, width, height,
                             monitor==nullptr?60: glfwGetVideoMode(monitor->getMonitor())->refreshRate);
    ...
    int getRefreshRate(){
        GLFWmonitor* monitor = glfwGetWindowMonitor(windowHandle);

        return monitor==nullptr?60:glfwGetVideoMode(monitor)->refreshRate;
    }
